...
- `backend/libs/my-agent/docs/competitive-intel-workflow.md`: workflow 示例设计
...
4. **停止原则**：
   ...
   - 要么都不停止（让用户手动管理）
   - 不要单独停止某一个服务
   - 例外：如果 `tester` 为了测试而启动了服务，则测试完成后应由 `tester` 负责一起停止前后端
...
- API: GraphQL-first
  ...
  - Do NOT add hand-written business GraphQL documents under `frontend/src/graphql/`; that directory is reserved for generated artifacts only.
