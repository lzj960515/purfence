...
app.get('/auth/callback', async (req, res) => {
  ...
  const errorStr = error ? String(Array.isArray(error) ? error[0] : error) : undefined;
  const errorDescStr = error_description ? String(Array.isArray(error_description) ? error_description[0] : error_description) : undefined;
  ...
  if (errorStr) {
    ...
    return res.redirect(`${FRONTEND_URL}/settings/providers?error=${encodeURIComponent(errorDescStr || errorStr)}`);
  }

  // Validate required parameters
  if (!codeStr || !stateStr) {
    console.error('[OAuth Callback] Missing parameters');
    return res.redirect(`${FRONTEND_URL}/settings/providers?error=${encodeURIComponent('Missing required parameters')}`);
  }
  ...
  try {
    ...
    console.log('[OAuth Callback] Sending GraphQL request to:', `${MAIN_BACKEND}/graphql`);
    ...
    console.log('[OAuth Callback] Full response:', JSON.stringify(result, null, 2));

    if (result.errors) {
      console.error('[OAuth Callback] Backend error:', JSON.stringify(result.errors, null, 2));
    ...
    res.redirect(`${FRONTEND_URL}/settings/providers?success=true&email=${encodeURIComponent(data.email)}`);
  } catch (err) {
    console.error('[OAuth Callback] Failed:', err);
    res.redirect(`${FRONTEND_URL}/settings/providers?error=${encodeURIComponent(`OAuth failed: ${err}`)}`);
  }
});
...
app.listen(PORT, () => {
  ...
  console.log(`[OAuth Callback Handler] OAuth endpoint: http://localhost:${PORT}/auth/callback`);
});
