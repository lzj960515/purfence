...
export class SlackRuntimeService implements OnModuleInit, OnModuleDestroy {
  ...
  private cleanupTimedOutSessions(): void {
    ...
    for (const [conversationId, session] of this.activeSessions) {
      if (now - session.createdAt > this.SESSION_TIMEOUT_MS) {
        ...
        // Also clean up the channel
        this.activeChannels.delete(session.channel);
      }
    }
  ...
  async onScheduledTaskStreamEnded(payload: ScheduledTaskStreamEndedEvent) {
    ...
    try {
      ...
      if (!finalText) {
        this.logger.warn(
          `No final message found for conversation: ${conversationId}`,
        );
        return;
      }

      await this.postMarkdownByToken(
        config.botToken,
        slackChannelId,
        finalText,
      );
  ...
  private async start(
    ...
    app.event('message', async ({ event }) => {
      ...
      // Determine the conversationId based on whether channel is processing
      let conversationId: string;
      if (!isChannelProcessing) {
        // Normal case: first message in channel, use channel id
        conversationId = event.channel;
      } else {
        // Channel is processing: concurrent message, use thread_ts ?? ts
      ...
      this.logger.log(
        `Created session: conversationId=${conversationId}, threadTs=${threadTs}`,
      );

      // Only send pre-reply for the first message (when !event.thread_ts)
      // This indicates a new thread while another is processing
      if (isChannelProcessing && !event.thread_ts) {
        try {
      ...
      SocketService.registerStreamMirror(conversationId, {
        handle: async (streamEvent, payload) => {
          await this.handleMirrorEvent(
            conversationId,
            streamEvent,
            payload as StreamPayload,
          );
        },
      ...
      try {
        ...
        this.logger.log(
          `Removed session due to error: conversationId=${conversationId}`,
        );
  ...
  private async handleMirrorEvent(
    ...
    try {
      ...
      const { app, channel, threadTs, isChannelProcessing } = session;
      ...
      if (type === 'tool_result') {
        ...
        await this.postTaskCardResult(
          ...
          threadTs,
          isChannelProcessing,
          session.taskMessageTs,
        );

        const artifacts = this.extractArtifactContents(payload.artifact);
        if (artifacts.length > 0) {
          this.logger.debug(
            `Posting ${artifacts.length} artifact(s) for conversation ${conversationId}`,
          );
          try {
            await this.purfenceSlackService.postArtifacts({
              client: app.client,
              channel,
              threadTs: isChannelProcessing ? threadTs : undefined,
              artifacts,
            });
          } catch (error) {
            this.logger.error(
  ...
  private async closeMirror(conversationId: string) {
    ...
    this.activeChannels.delete(session.channel);
    this.logger.log(
      `Closed and removed session: conversationId=${conversationId}`,
    );
  }
  ...
  private async flushCurrentBlock(conversationId: string) {
    ...
    if (session.currentType === 'thinking' || session.currentType === 'text') {
      // 先不发thinking
      if (
        !session.markdownBuffer.trim() ||
        session.currentType === 'thinking'
      ) {
        session.currentType = '';
      ...
      try {
        // Only pass thread_ts when channel was already processing (new thread case)
        const postMessageParams: Parameters<
          typeof session.app.client.chat.postMessage
        >[0] = {
        ...
        if (session.isChannelProcessing) {
          postMessageParams.thread_ts = session.threadTs;
        }
        await session.app.client.chat.postMessage(postMessageParams);
      } catch (error) {
        this.logger.error(
  ...
  private async postTaskCardStart(
    ...
    try {
      const postMessageParams: Parameters<
        typeof app.client.chat.postMessage
      >[0] = {
        channel,
        text: title,
        blocks: [
      ...
      if (isChannelProcessing) {
        postMessageParams.thread_ts = threadTs;
      }
      const result = await app.client.chat.postMessage(postMessageParams);
  ...
  private async postTaskCardResult(
    ...
    if (!ts) {
      try {
        ...
        await app.client.chat.postMessage(postMessageParams);
      } catch (error) {
        this.logger.error(
  ...
  private async postErrorBlock(
    ...
    try {
      ...
      // Only pass thread_ts when channel was already processing (new thread case)
      if (isChannelProcessing && threadTs) {
        postMessageParams.thread_ts = threadTs;
      }
      await app.client.chat.postMessage(postMessageParams);
    } catch (error) {
